...
int input_info(STUDENT *stus,int stu_num)
{
    ...
    for(i=0; i<stu_num; i++)
    {
        printf("%s,%s,%d,%d\n",stus[i].id,stus[i].name,stus[i].score,stus[i].absent);
    }
    return stu_num;
}
...
void random_call(STUDENT *stus,int stu_num)
{
    ...
    srand(time(NULL));
    call_num = rand()%CMAX;
    int *call_index = (int *)malloc(NAME_LEN*sizeof(int));
    if(call_index == NULL)
    {
    ...
    {
        memset(call_index,0,NAME_LEN * sizeof(int));
        printf("=======\n");
...
int main()
{
    ...
    scanf(" %d",&stu_num);
    STUDENT *stus = (STUDENT*)malloc(sizeof(STUDENT) * stu_num);
    if(stus == NULL)
    {
        printf("No enough memory.\n");
        exit(0);
    ...
    {
        memset(stus,0,sizeof(STUDENT)*stu_num);
